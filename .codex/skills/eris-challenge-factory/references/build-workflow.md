...
4. Require a measured Scout evidence card with verdict **Proceed**. It must establish the exact input/target join, native or independently derived label provenance, official access and redistribution terms, raw size, post-filter sample and family counts, post-anonymization answerability, and at least a bounded shortcut/baseline result.
5. If the handoff is **Pilot** or **Hold**, or merely says the user approved the concept, do not start a full build. Run only the explicitly authorized bounded pilot, write its evidence, and return a measured Proceed/Hold/Reject verdict.
6. Re-run the novelty, source/license, source-size, CPU, and lookup-leak gates before implementation.
7. Write `SOURCE_VERIFICATION.md` early. Stop if a mandatory claim cannot be verified. Never allow parser-generated outputs to certify the parser's own labels.
...
- For hidden-source or lookup-sensitive builds, do not put exact source names, source URLs, paper titles/DOIs, license names, attribution requests, or dataset-creator breadcrumbs in the platform-visible problem description. Keep source/license attribution in dataset metadata, source verification, and acquisition docs.
...
Run the challenge-builder audit helper when available, then manually check every live checkpoint. Inspect real prepared artifacts and representative media/signals. For numeric payloads such as `.npz`, `.npy`, density traces, embeddings, masks, or feature tensors, verify documented keys/shapes, all finite values, non-placeholder variance where applicable, and no duplicate payload hashes across train/test. Verify raw upload contents and platform-visible extracted layout. Update status to `ready_for_review` only when no required work remains; otherwise use `blocked` with evidence.
