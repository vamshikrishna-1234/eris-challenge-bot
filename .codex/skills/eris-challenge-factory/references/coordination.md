...
- Use only those values in `status`. Put finer platform state in `stage` and the reusable cause in `reason_code`/`status_note`; do not invent statuses such as `rejected_scale` or `platform_revision_blocked_*` because they make portfolio audits unreliable.
- Use `blocked` only for a potentially reversible external or implementation condition. Use `rejected` when measured evidence invalidates the current formulation. Use `researching` for Pilot work and `claimed` only after the Proceed validator passes.
